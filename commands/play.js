...
//Khmer
const youtubeapi2 = new YouTubeAPI('YOUTUBEAPI2');
//Khmer
const youtubeapi3 = new YouTubeAPI('YOUTUBEAPI3');
  ...
  async execute(message, args) {
    ...
    if (urlValid) {
      ...
      try {
        ...
        try{
          console.log('P API 1 Fail Starting API 2');
          const results = await youtubeapi2.searchVideos(search, 1);
          songInfo = await ytdl.getInfo(results[0].url);
          song = {
            title: songInfo.videoDetails.title,
            url: songInfo.videoDetails.video_url,
          ...
          try{
            console.log('P API 2 Fail Starting API 3');
            const results = await youtubeapi3.searchVideos(search, 1);
            songInfo = await ytdl.getInfo(results[0].url);
            song = {
              title: songInfo.videoDetails.title,
              url: songInfo.videoDetails.video_url,
        ...
        try{
          console.log('P API 2 Fail Starting API 3 EV2');
          const results = await youtubeapi3.searchVideos(search, 1);
          songInfo = await ytdl.getInfo(results[0].url);
          song = {
            title: songInfo.videoDetails.title,
